...
class Qwen3ReasoningParser(DeepSeekR1ReasoningParser):
    def extract_reasoning(self, model_output, request):
        ...
        if (
            ...
            1. Since Qwen3.6 has thinking off modes modulated by "enable_thinking=false" in the chat template kwargs, this change instead properly places the content in cases where there is no thinking enabled via config.
            2. There are rare cases where the model will output only reasoning without an end-think token `<tool_call>` (e.g. reasoning exceeds max length), which results in empty content returned. End users may want to unilaterally avoid such cases and always have a content response even if the model does not finish its reasoning.
            """
